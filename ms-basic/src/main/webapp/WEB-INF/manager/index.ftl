            ...
            <el-scrollbar style="height:100%">
                <el-menu :default-active="asideMenuActive" class="el-menu-vertical-demo"
                    ...
                    <div class="ms-admin-logo" :style="{display: 'flex','padding-left': (collapseMenu?'5px':'22px')}">
                        <img :src="ms.base+'/static/images/mcms.png'"/>
                    ...
                    <!--  当没有菜单显示时显示提示图片  -->
                    <img v-if="asideMenuList.length==0&&!state.menu" src="${base}/static/images/tip.png" class="ms-admin-tip" />
                    <!-- 收缩按钮 -->
                </el-menu>
            </el-scrollbar>
        </el-aside>
    ...
    var indexVue = new Vue({
        ...
        data: function () {
            return {
                ...
                menuState: 0, //收藏菜单是否为初始状态，0是初始，1是用户主动修改过后
        ...
        watch: {
            ...
            parentMenuList: function (n, o) {
                ...
                that.asideMenuList.forEach(function (item) {
                    that.markList.forEach(function (_item) {
                        if (item.id == _item.id) {
                            item.isSingle = _item.isSingle;
                        }
                    })
                })
            },
        ...
        methods: {
            markMenu: function (item) {
                ...
                localStorage.setItem(that.managerInfo.managerName + "markList", JSON.stringify(this.markList))
                this.callbackFun = this.markList;
            },
            cancelMarkMenu: function (item) {
                ...
                localStorage.setItem(that.managerInfo.managerName + "markList", JSON.stringify(this.markList))
                //取消收藏后，收藏菜单初始状态修改
                localStorage.setItem(that.managerInfo.managerName + "menuState",1)
                this.callbackFun = this.markList;
            },
            //  打开修改密码模态框
            openModal: function () {
            ...
            list: function () {
                ...
                ms.http.get(ms.manager + "/basic/model/list.do")
                    .then(function (res) {
                        ...
                        //当收藏为0且为初次打开页面时
                        if(that.markList.length == 0 && that.menuState == 0){
                            //展示全部页面
                            that.menuList.map(function (item, index, arr) {
                                if(!item.modelId){
                                    that.markList.push({
                                        title: item.modelTitle,
                                        icon: item.modelIcon,
                            ...
                            // 取出存入的管理员名称
                            var localMarkList = localStorage.getItem(that.managerInfo.managerName + "markList");
                            // 判空
                            if (!localMarkList || localMarkList.length<0){
                                //先赋空，防止push重复的,
                                that.markList = [];
                                //展示全部页面
                                that.menuList.map(function (item, index, arr) {
                        ...
                        that.markList.forEach(function (item) {
                            var children = [];
                            that.menuList.forEach(function (_item) {
                                if (_item.modelId == item.id) {
                                    children.push(_item)
                                }
                            })
                            // 若子菜单不是功能导航
                        ...
                        localStorage.setItem(that.managerInfo.managerName + "markList", JSON.stringify(that.markList))
                    }, function (err) {
                        that.$notify({
                            title: '错误',
                            message: err,
            ...
            managerGet: function () {
                ...
                ms.http.get(ms.manager + "/basic/manager/info.do")
                    .then(function (res) {
                        ...
                        // 根据管理员名称取出缓存数据
                        var markList = localStorage.getItem(that.managerInfo.managerName + "markList");
                        var menuState = localStorage.getItem(that.managerInfo.managerName + "menuState");
                        if (markList) {
                            that.markList = JSON.parse(markList)
                        }
                        if (menuState) {
                            that.menuState = menuState
                        }
                        // 请求菜单数据
                        that.list();
                    }, function (err) {
