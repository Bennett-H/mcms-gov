...
<body>
	<div id="index" style="width: 100%;"  v-cloak>
		...
		<el-main class="ms-container">
			<el-table v-loading="loading" class="ms-table-pagination" ref="multipleTable" border="true"  :data="dataList" tooltip-effect="dark" @selection-change="handleSelectionChange" :max-height="tableHeight" >
                ...
                <el-table-column label="公众号名称" width="180">
	                <template slot-scope="scope">
							{{ scope.row.weixinName }}
					</template>
                </el-table-column>
                <el-table-column label="微信号" width="180">
                	<template slot-scope="scope">
                			{{ scope.row.weixinNo }}
					</template>
                </el-table-column>
            	...
            	<el-table-column label="操作" align="center" width="180">
            		<template slot-scope="scope">
	            		...
	            		<a :href="ms.manager + '/mweixin/'+ parseInt(scope.row.id) + '/weixin.do'">进入公众号</a>
						</@shiro.hasPermission>
						<@shiro.hasPermission name="weixin:update">
	            		<a :href="ms.manager + '/mweixin/form.do?weixinId='+ parseInt(scope.row.id)">编辑</a>
						</@shiro.hasPermission>
            		</template>
            	</el-table-column>
            </el-table>
	...
	methods: {
		...
		list: function() {
			...
			setTimeout(function(){
				ms.http.get(ms.manager + "/mweixin/list.do").then(function(data){
					if (!data.result || data.data.total <=0){
						that.loading = false;
						that.emptyText='暂无数据'
						that.dataList =[];
					}else {
        ...
        del: function(){
        	...
        	that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
					    	...
					    	ms.http.post(ms.manager + "/mweixin/delete.do", that.selData,{
            					headers: {
