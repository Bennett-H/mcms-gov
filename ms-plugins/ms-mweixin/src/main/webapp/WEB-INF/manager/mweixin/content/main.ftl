...
<div id="index"  v-cloak>
    ...
    <el-container class="index-menu">
        ...
        <iframe :src="action" id="contentIframe" class="ms-iframe-style ms-loading">
        </iframe>
    </el-container>
</div>
</body>
...
<script>
    var indexVue = new Vue({
        ...
        methods: {
            handleNodeClick: function (data) {
                if (data.categoryType == '1') {
                    this.action = ms.manager + "/mweixin/content/index.do?categoryId=" + data.id+"&leaf="+data.leaf;
                } else if (data.categoryType == '2') {
                    this.action = ms.manager + "/mweixin/content/form.do?categoryId=" + data.id + "&type=2";
                    //id=0时为最顶级节点全部节点
                } else if (data.id == 0){
                    this.action = ms.manager + "/mweixin/content/index.do?leaf=false";
                }
            },
            treeList: function () {
                var that = this;
        ...
        mounted: function () {
            this.action = ms.manager + "/mweixin/content/index.do";
            this.treeList();
        }
    });
</script>
