...
<script>
    var replyVue = new Vue({
        ...
        methods: {
            ...
            back: function () {
                var that = this;
                var dict = that.dictList.find(function (value) {
                    return value.dictValue == that.comment.dataType;
                })
                location.href = ms.manager + "/comment/index.do?dataType="+dict.dictLabel;
            },
            reply:function (row){
                this.replyComment = {}
                this.commentData = {};
            ...
            save:function (id) {
                ...
                var data = this.dictList.find(function (data) {
                    return data.dictValue == that.comment.dataType;
                })
                ms.util.openSystemUrl("/comment/form.do?dataType=" + data.dictLabel  + "&returnUrl=" + location.href + (id ? "&id="+id : ""));
            },
            //保存
            saveOrUpdate: function (row) {
                ...
                ms.http.post(ms.manager + "/comment/save.do", that.replyComment).then(function (res) {
                    if (res.result) {
                        that.$notify({
                            title: '成功',
                            message: '评论回复成功!',
            ...
            get: function (id) {
                var that = this;
                ms.http.get(ms.manager + "/comment/getComment.do", {
                    id: id
                }).then(function (res) {
                    if (res.data.id > 0) {
                    ...
                    if (res.data.total > 0) {
                        ...
                        commentDataList.forEach(function (commentData){
                            ...
                            commentDataList.forEach(function (comment){
                                ...
                                if (commentData.id == comment.commentId ){
                                    // 会员评论
