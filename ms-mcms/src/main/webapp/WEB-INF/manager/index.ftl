<!-- 主页 -->
<!-- 2021-12-27:覆写basic-UI使用自定义配置获取,增加uiConfig变量-->
<!-- 主页 -->
<!DOCTYPE html>
<html>
<head>
    ...
    <#--    站群切换-->
<#--    <#include '/site/component/ms-site-header.ftl'/>-->
            ...
            <el-scrollbar style="height:100%">
                <el-menu :default-active="asideMenuActive" class="el-menu-vertical-demo"
                    ...
                    <div class="ms-admin-logo" :style="{display: 'flex','padding-left': (collapseMenu?'5px':'22px')}">
                        <img :src="uiConfig.uiLogo"/>
                        <div class="class-1" v-show="!collapseMenu">
                            <div v-if="uiConfig.uiVersionNumber" class="class-2">v</div>
                            {{uiConfig.uiVersionNumber}}
                        </div>
                    </div>
                    ...
                    <!-- 子菜单 -->
                    <el-menu-item v-else :index="menu.id+''" :data-index="menu.id"
                                  :key='menu.modelId'
                ...
                <div class="ms-admin-header-right">
                    ...
                    <template slot="reset-password">
                      <el-menu-item
                        ...
                        <el-menu-item
                                style="display: flex;align-items: center" @click.native='openPhoneModal'>
                    ...
                    <!--mstore按钮-->
                    <ms-store v-if="uiConfig.uiStore!='false'" ref="storec" client="${client}"></ms-store>
                </div>
    ...
    var indexVue = new Vue({
        ...
        methods: {
            ...
            openPhoneModal: function () {
                var that = this
                if(event.target.innerText.indexOf('绑定手机') > -1 ){
                    resetPhoneVue.getManagerPhone()
                    resetPhoneVue.resetPhoneForm.managerName = that.managerInfo.managerName
                    resetPhoneVue.isShow = true
                } else {
                    this.exitSystem();
                }
            },
            ...
            getUIConfig: function () {
                var that = this;
                ms.mdiy.config("后台UI配置", '',true).then(function (res){
                    if(res.result){
                        that.uiConfig = res.data
                        res.data.uiLogo && JSON.parse(res.data.uiLogo) ? that.uiConfig.uiLogo = JSON.parse(res.data.uiLogo)[0].url : that.uiConfig.uiLogo = "${base}/static/images/logo-white.png"
                    } else {
                        that.uiConfig = {
                            uiVersionNumber: '',
                            uiLogo: "${base}/static/images/logo-white.png",
                            uiStore: "true"
                        }
                    }
                }).catch(function(err){
            ...
            //     if(res.result){
            //         that.uiConfig = res.data
            //         JSON.parse(res.data.uiLogo) ? that.uiConfig.uiLogo = JSON.parse(res.data.uiLogo)[0].url : that.uiConfig.uiLogo = "${base}/static/images/logo-white.png"
            //       } else {
            //         that.uiConfig = {
            //           uiVersionNumber: '',
            //           uiLogo: "${base}/static/images/logo-white.png",
