...
<div id="index" v-cloak class="ms-index">
	<el-header class="ms-header" height="50px">
		<el-col :span="24">
			...
			<el-tooltip class="item" effect="dark" content="如果配置数据有修改，数据获取不正确，尝试刷新缓存" placement="bottom">
				<el-button type="primary" plain style="float: right; margin-right: 8px"
	...
	var indexVue = new Vue({
		...
		methods: {
			...
			refreshCache: function() {
				var that = this;
					ms.http.post(ms.manager + "/mdiy/configCache/refreshCache.do").then(function (data) {
						if (data.result) {
							that.$notify({
								title: '成功',
								type: 'success',
