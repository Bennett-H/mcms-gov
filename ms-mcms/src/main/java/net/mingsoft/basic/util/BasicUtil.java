 ...
 * @date 2020年7月2日
 * 历史修订:
 * 2021-12-27 覆写basic getRealTemplatePath更改为使用自定义配置
 * 2022-3-17 修改getApp方法,先通过url获取app
 */
public class BasicUtil {
                ...
                if (MapCacheUtil.get(BasicUtil.getDomain()) != null){
                    ...
                    _app = appBiz.getOne(
                            Wrappers.<AppEntity>lambdaQuery()
                                    .like(AppEntity::getAppUrl, BasicUtil.getDomain()).orderByAsc(AppEntity::getId), false);
                    if (_app == null){
    ...
    public static AppEntity getApp(String url) {
        ...
        wrapper.like(AppEntity::getAppUrl, url);
        wrapper.orderByAsc(AppEntity::getId);
        return appBiz.getOne(wrapper, false);
    }
     ...
     * 获取整型值数组
     *
     * @param param 参数名称，如果param参数的值本身就是数组，将会忽略split参数
     * @param split 分割符号
     * @return 不存在返回null
     */
