...
public class CategoryBizImpl extends BaseBizImpl<ICategoryDao, CategoryEntity> implements ICategoryBiz {
    ...
    public void updateEntity(CategoryEntity entity) {
        ...
        if (StrUtil.isNotBlank(pingYin)) {
            CategoryEntity category = new CategoryEntity();
            category.setCategoryPinyin(pingYin);
            CategoryEntity categoryBizEntity = getEntity(category);
            //拼音存在则拼接id
    ...
    public void delete(String categoryId) {
        ...
        if (category != null) {
            ...
            //判断删除的是否为主节点
            if (parentNode != null) {
                LambdaUpdateWrapper<CategoryEntity> updateWrapper = new UpdateWrapper<CategoryEntity>().lambda();
                //如果没有子节点进行更新代码
                if (childNode.size() == 1) {
                    updateWrapper.eq(CategoryEntity::getId, parentNode.getId());
                    updateWrapper.set(CategoryEntity::getLeaf, 1);
    ...
    private void setTopId(CategoryEntity entity) {
        ...
        // 此处修改会导致标签错误，目前统一为0，如要修改需要同事修改标签
        entity.setTopId("0");
    }
     ...
     */
    @Override
    public List<CategoryEntity> queryCategoryIgnoreSite() {
        return categoryDao.queryCategoryIgnoreSite(null);
    }
     ...
     */
    @Override
    public List<CategoryEntity> queryCategoryIgnoreSite(String[] ids) {
        return categoryDao.queryCategoryIgnoreSite(ids);
    }

    @Override
    public void changeType(CategoryEntity categoryEntity, String targetCategoryType) {
        // 转换为单篇
        if (CategoryTypeEnum.COVER.toString().equalsIgnoreCase(targetCategoryType)){
            ...
            categoryEntity.setCategoryUrls("");
            categoryEntity.setCategoryListUrls("");
            // 转换为链接
        } else if (CategoryTypeEnum.LINK.toString().equals(targetCategoryType)){
            contentDao.deleteEntityByCategoryIds(new String[]{categoryEntity.getId()});
            categoryEntity.setCategoryUrls("");
            categoryEntity.setCategoryListUrls("");
            // 转换为列表
        } else if (CategoryTypeEnum.LIST.toString().equals(targetCategoryType)) {
            categoryEntity.setCategoryUrls("");
        }

        categoryEntity.setCategoryType(targetCategoryType);
        categoryEntity.setCategoryDiyUrl("");
