...
<script>
    var form = new Vue({
        ...
        methods: {
            back: function () {
                // location.href = ms.manager + "/mdiy/dictData/index.do?dictType=" + this.form.dictType;
                ms.util.openSystemUrl("/mdiy/dict/data/index.do?dictType=" + this.form.dictType);
            },
            save: function () {
                var that = this;
                var url = ms.manager + "/mdiy/dict/data/save.do";

                if (that.form.id > 0) {
                    url = ms.manager + "/mdiy/dict/data/update.do";
                }
                    ...
                    if (valid) {
                        ...
                        ms.http.post(url, data).then(function (data) {
                            if (data.result) {
                                ...
                                // location.href = ms.manager + "/mdiy/dictData/index.do?dictType=" + that.form.dictType;
                                ms.util.openSystemUrl("/mdiy/dict/data/index.do?dictType=" + that.form.dictType);
                            } else {
                                that.$notify({
                                    title: '失败',
                                    message: data.msg,
            ...
            get: function (id) {
                var that = this;
                ms.http.get(ms.manager + "/mdiy/dict/data/get.do", {
                    "id": id
                }).then(function (data) {
                    if (data.result) {
                        that.form = data.data;
