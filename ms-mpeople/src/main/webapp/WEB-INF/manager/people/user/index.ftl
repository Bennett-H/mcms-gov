...
<div id="index" v-cloak class="ms-index">
    ...
    <div class="ms-search" style="padding: 20px 10px 20px 10px;">
        <el-row>
            <el-form :model="form" ref="searchForm" :rules="rules" label-width="80px" size="mini">
                <el-row>
                    <el-col :span="8">
    ...
    <el-main class="ms-container">
        <el-table v-loading="loading" ref="multipleTable" height="calc(100vh-68px)" class="ms-table-pagination" border :data="treeList"
            ...
            <el-table-column label="注册时间" width="200" align="center" prop="peopleDateTime">
            </el-table-column>
...
<script>
    var indexVue = new Vue({
        ...
        data: {
            ...
            rules: {peopleId:[{ max: 9, message: '长度不能超过9位', trigger: 'blur' }]}
        },
        methods: {
            //查询列表
            list: function () {
                ...
                that.$refs.searchForm.validate(function (valid) {
                    if (valid){
                        setTimeout(function () {
                            ms.http.get(ms.manager + "/people/peopleUser/auditList.do",Object.assign({}, that.form, page))
                                .then(function (data) {
                                    if(data.result){
